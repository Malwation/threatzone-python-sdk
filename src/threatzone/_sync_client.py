"""Synchronous Threat.Zone client implementation."""
...
from ._client import FileInput, SyncHTTPClient
...
from ._streaming import DownloadResponse
from .types import (
    ArtifactsResponse,
    BehaviourOs,
    BehavioursResponse,
...
class ThreatZone:
    """Synchronous client for the Threat.Zone Public API."""
    ...
    def __init__(
        ...
        *,
        ...
        """
        Initialize the Threat.Zone client.
        ...
        Args:
            ...
            verify_ssl: Whether to verify SSL certificates. Set to False for self-signed
                ...
                ``http_client`` is supplied.
            http_client: Optional pre-configured ``httpx.Client`` to use for every
    ...
    def get_metafields(self, scan_type: str | None = None) -> Metafields | list[MetafieldOption]:
        """
        ...
        """
        if scan_type:
            response = self._http.get(f"/config/metafields/{scan_type}")
            return [MetafieldOption.model_validate(item) for item in response.json()]
        else:
            response = self._http.get("/config/metafields")
            return Metafields.model_validate(response.json())

    def get_environments(self) -> list[EnvironmentOption]:
    ...
    def create_open_in_browser_submission(
        ...
        *,
        ...
        response = self._http.post("/submissions/open_in_browser", json=payload)
        return SubmissionCreated.model_validate(response.json())
    ...
    def get_submission(self, uuid: str) -> Submission:
        """
        ...
        """
        response = self._http.get(f"/submissions/{uuid}")
        return Submission.model_validate(response.json())

    def search_by_sha256(self, sha256: str) -> list[Submission]:
        """
        ...
        """
        response = self._http.get(f"/submissions/search/sha256/{sha256}")
    ...
    def get_overview_summary(self, uuid: str) -> OverviewSummary:
        """
        ...
        """
        response = self._http.get(f"/submissions/{uuid}/summary")
    ...
    def get_extracted_configs(self, uuid: str) -> ExtractedConfigsResponse:
        """
        ...
        """
        response = self._http.get(f"/submissions/{uuid}/extracted-configs")
        return ExtractedConfigsResponse.model_validate(response.json())

    def get_artifacts(self, uuid: str) -> ArtifactsResponse:
        """
        ...
        """
        response = self._http.get(f"/submissions/{uuid}/artifacts")
        return ArtifactsResponse.model_validate(response.json())

    def get_eml_analysis(self, uuid: str) -> list[EmlAnalysis]:
        """
        ...
        """
        response = self._http.get(f"/submissions/{uuid}/eml-analysis")
        return [EmlAnalysis.model_validate(item) for item in response.json()]

    def get_mitre_techniques(self, uuid: str) -> MitreResponse:
        """
        ...
        """
        response = self._http.get(f"/submissions/{uuid}/mitre")
    ...
    def get_cdr_results(self, uuid: str) -> CdrResponse:
        ...
        """
        response = self._http.get(f"/submissions/{uuid}/cdr")
        return CdrResponse.model_validate(response.json())

    def get_signature_check_results(self, uuid: str) -> SignatureCheckResponse:
        """
        ...
        Raises:
            ReportUnavailableError: When the static report is not yet available.
        """
        response = self._http.get(f"/submissions/{uuid}/signature-check")
    ...
    def get_processes(self, uuid: str) -> ProcessesResponse:
        """
        ...
        """
        response = self._http.get(f"/submissions/{uuid}/processes")
        return ProcessesResponse.model_validate(response.json())

    def get_process_tree(self, uuid: str) -> ProcessTreeResponse:
        """
        ...
        Raises:
            ReportUnavailableError: When the dynamic report is not yet available.
        """
        response = self._http.get(f"/submissions/{uuid}/processes/tree")
    ...
    def get_url_analysis(self, uuid: str) -> UrlAnalysisResponse:
        """
    ...
    def get_network_summary(self, uuid: str) -> NetworkSummary:
        """
        ...
        """
        response = self._http.get(f"/submissions/{uuid}/network/summary")
    ...
    def download_sample(self, uuid: str) -> DownloadResponse:
        """
        ...
        Returns:
            DownloadResponse for streaming or saving the file.
        """
        return self._http.get_stream(f"/submissions/{uuid}/download/sample")

    def download_artifact(self, uuid: str, artifact_id: str) -> DownloadResponse:
        """
        ...
        Returns:
            DownloadResponse for streaming or saving the file.
        """
        return self._http.get_stream(f"/submissions/{uuid}/download/artifact/{artifact_id}")

    def download_pcap(self, uuid: str) -> DownloadResponse:
        """
        ...
        Returns:
            DownloadResponse for streaming or saving the file.
        """
        return self._http.get_stream(f"/submissions/{uuid}/download/pcap")

    def download_yara_rule(self, uuid: str) -> DownloadResponse:
        """
        ...
        Returns:
            DownloadResponse for streaming or saving the file.
        """
        download = self._http.get_stream(f"/submissions/{uuid}/download/yara-rule")
    ...
    def download_html_report(self, uuid: str) -> DownloadResponse:
        """
        ...
        Returns:
            DownloadResponse for streaming or saving the file.
        """
        return self._http.get_stream(f"/submissions/{uuid}/download/html-report")

    def download_cdr_result(self, uuid: str) -> DownloadResponse:
        """
        ...
        Returns:
            DownloadResponse for streaming or saving the file.
        """
        return self._http.get_stream(f"/submissions/{uuid}/download/cdr")
